...
createServer({
  ...
  seeds(server) {
    const matt = server.create("actor", { name: "Matthew McConaughey" }),
      anne = server.create("actor", { name: "Anne Hathaway" }),
      leo = server.create("actor", { name: "Leonardo Dicarpio" }),
      tom = server.create("actor", { name: "Tom Hardy" }),
      cillian = server.create("actor", { name: "Cillian Muphry" });
